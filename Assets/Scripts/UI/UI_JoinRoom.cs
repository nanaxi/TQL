...
public class UI_JoinRoom : MonoBehaviour
{
    ...
    void Init()
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            ClickNum[i] = ThisTrans.Find("Mask/Content/BG/NumberButton/Btn_Number_" + i).GetComponent<Button>();
        }
        for (int i = 0; i < 6; i++)
        {
            ShowNum[i] = ThisTrans.Find("Mask/Content/BG/Input_Num/" + i).GetComponentInChildren<Text>();
        }
        backInput = ThisTrans.Find("Mask/Content/BG/NumberButton/Btn_Number_12").GetComponent<Button>();
        reInput = ThisTrans.Find("Mask/Content/BG/NumberButton/Btn_Number_11").GetComponent<Button>();
    }
    void DeFault()
    {
        if (Close != null)
